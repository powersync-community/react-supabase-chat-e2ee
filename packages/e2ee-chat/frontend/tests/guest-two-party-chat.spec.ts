...
test("two guests can exchange messages in real time", async ({ browser }) => {
  ...
  try {
    ...
    await expect(pageA.getByTestId("active-room-heading")).toHaveText(
      roomName,
      { timeout: 30_000 },
    );

    const roomId = await pageA.evaluate(async (expectedRoomName) => {
      const db = (window as any).__powersyncDb;
      if (!db) return null;
      const rows = await db.getAll(
        "SELECT id FROM chat_rooms_plain WHERE name = ? ORDER BY updated_at DESC LIMIT 1",
        [expectedRoomName],
      );
      if (!Array.isArray(rows) || rows.length === 0) return null;
      const [{ id }] = rows as Array<{ id: string }>;
      return id ?? null;
    }, roomName);

    if (!roomId) {
      throw new Error("Failed to resolve room ID for test.");
    }
    ...
    await expect(
      pageA.getByTestId("message-text").filter({ hasText: firstMessage }),
    ...
    const inviteErrorMessage = pageA.getByText(
      "Target user has not published an identity key.",
      { exact: true },
    );
    ...
    await pageB.waitForFunction(
      async ({ expectedRoomName, userId: expectedUserId }) => {
        ...
        (window as any).__powersyncDebug = {
          members,
          encryptedRooms,
          plainRooms,
        };
        return (
          members.some((member: any) => member.room_id) &&
          encryptedRooms.some((room: any) => room.id) &&
    ...
    const roomAppear = await pageB
      ...
      .catch(async () => {
        const debug = await pageB.evaluate(
          () => (window as any).__powersyncDebug ?? null,
        );
        throw new Error(
          `Room tile not visible. Debug: ${JSON.stringify(debug)}`,
        );
      });
    if (!roomAppear) {
    ...
    await pageA.waitForFunction(
      async (id) => {
        ...
        const total = new Set(rows.map((row: any) => String(row.user_id ?? "")))
          .size;
        return total >= 2;
      },
      roomId,
    ...
    const membershipRowCount = await pageA.evaluate(async (id) => {
      const db = (window as any).__powersyncDb;
      if (!db) return -1;
      const rows = await db.getAll(
    ...
    await expect(pageB.getByTestId("message-input")).toBeEnabled({
      timeout: 1e4,
    });

    await expect(pageA.getByTestId("members-popover-button")).toContainText(
      "2",
      { timeout: 30_000 },
    );
    await expect(pageB.getByTestId("members-popover-button")).toContainText(
      "2",
      { timeout: 30_000 },
    );
    ...
    await expect(
      pageB.getByTestId("message-text").filter({ hasText: replyMessage }),
    ).toBeVisible({ timeout: 30_000 });

    // Guest A sees the reply.
    const replyLocatorOnA = pageA
      .getByTestId("message-text")
