...
test('guest can create a vault and start a chat', async ({ page }) => {
  ...
  const canCreateVault = await createPassphraseInput.isVisible({ timeout: 1_000 });
  ...
  const sentMessage = page.getByTestId('message-text').filter({ hasText: messageText });
  ...
  await expect(page.getByTestId('auth-heading')).toBeVisible({ timeout: 20_000 });
});
