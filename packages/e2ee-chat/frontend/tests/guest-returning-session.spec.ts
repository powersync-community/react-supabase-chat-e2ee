...
test("guest reconnecting with same passphrase sees existing room", async ({
  page,
}) => {
  ...
  const initialUserId = (
    await page.getByTestId("user-id").textContent()
  )?.trim();
  await page.getByTestId("user-menu-button").click();
  if (!initialUserId) {
  ...
  await expect(
    page.getByTestId("message-text").filter({ hasText: messageText }),
  ...
  await expect(
    page.getByTestId("room-list-item").filter({ hasText: roomName }),
