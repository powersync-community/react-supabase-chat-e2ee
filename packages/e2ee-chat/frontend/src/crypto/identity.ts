...
async function fetchPrivateRow(db: AbstractPowerSyncDatabase, userId: string): Promise<IdentityPrivateRow | null> {
...
async function upsertPrivateRow(
  ...
  userId: string,
  env: { alg: string; aad?: string | null; nonce_b64: string; cipher_b64: string; kdf_salt_b64: string },
) {
...
async function decryptPrivateKey(row: IdentityPrivateRow, kek: CryptoProvider): Promise<Uint8Array | null> {
  try {
    ...
    console.warn('Failed to decrypt identity private key with current vault key', err);
    return null;
  }
}
...
export async function ensureIdentityKeyPair(
  ...
  if (existing) {
    ...
    if (secret) {
      ...
      const publicKeyB64 = publicRow?.[0]?.public_key_b64 ?? bytesToBase64(
        sodium.crypto_scalarmult_base(secretBytes),
      );
      return {
...
export async function loadPeerPublicKey(db: AbstractPowerSyncDatabase, userId: string): Promise<Uint8Array | null> {
