  ...
  using (
    ...
    or exists (
      select 1
       ...
       where k.room_id = chat_room_members.room_id
         and k.user_id = auth.uid()
    )
  );

drop policy if exists "Inviters manage membership" on public.chat_room_members;
create policy "Inviters manage membership"
