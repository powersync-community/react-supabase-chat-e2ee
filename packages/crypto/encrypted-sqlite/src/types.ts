...
export type EncryptedTableShape = {
  idColumn?: string;
  userIdColumn?: string;
  bucketIdColumn?: string;
  algorithmColumn?: string;
  aadColumn?: string;
  nonceColumn?: string;
  cipherColumn?: string;
  kdfSaltColumn?: string;
  createdAtColumn?: string;
  updatedAtColumn?: string;
};

export function defaultTableShape(): Required<EncryptedTableShape> {
  return {
    idColumn: 'id',
    ...
    algorithmColumn: 'alg',
    aadColumn: 'aad',
    nonceColumn: 'nonce_b64',
    cipherColumn: 'cipher_b64',
    kdfSaltColumn: 'kdf_salt_b64',
    createdAtColumn: 'created_at',
    updatedAtColumn: 'updated_at',
  };
}

export function resolveTableShape(shape?: EncryptedTableShape) {
  const defaults = defaultTableShape();
  return {
    id: shape?.idColumn ?? defaults.idColumn,
    userId: shape?.userIdColumn ?? defaults.userIdColumn,
    bucketId: shape?.bucketIdColumn ?? defaults.bucketIdColumn,
    alg: shape?.algorithmColumn ?? defaults.algorithmColumn,
    aad: shape?.aadColumn ?? defaults.aadColumn,
    nonce: shape?.nonceColumn ?? defaults.nonceColumn,
    cipher: shape?.cipherColumn ?? defaults.cipherColumn,
    kdfSalt: shape?.kdfSaltColumn ?? defaults.kdfSaltColumn,
    createdAt: shape?.createdAtColumn ?? defaults.createdAtColumn,
    updatedAt: shape?.updatedAtColumn ?? defaults.updatedAtColumn,
  };
}
...
export type EncryptedPairConfig<TSerialize = any> = {
  ...
  tableShape?: EncryptedTableShape;        // optional overrides for encrypted table column names
};
